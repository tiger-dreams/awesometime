...
import { ALL_THEME_KEYS } from './themes.js';
...
export function renderFromQuery(query = {}, now = new Date()) {
  ...
  // Accept any known theme (built-in or named), fallback to 'dark'
  const theme = ALL_THEME_KEYS.includes(query.theme) ? query.theme : 'dark';
  ...
  // Backward compat: ?color= maps to accent. New params: border, text, dim, barBg, accent2, bg
  const colors = {
    accent: query.color,
    accent2: query.accent2,
    bg: query.bg,
    border: query.border,
    text: query.text,
    dim: query.dim,
  ...
  const reduceMotion = query.motion === 'reduce';
  ...
  if (type === 'countdown' || type === 'dayssince') {
    ...
    if (query.style === 'badge') {
      return {
        status: 200,
        svg: renderCountdownBadge({ title: label, days, isPast, theme, colors, font }),
      };
    }
    ...
    return {
      status: 200,
      svg: renderCountdownCard({ title, days, hours, minutes, seconds, isPast, theme, colors, font, reduceMotion }),
    };
  }
