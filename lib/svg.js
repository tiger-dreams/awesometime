import { THEME_KEYS, NAMED_THEMES } from './themes.js';
...
// Merge built-in themes with named themes. Named themes are all dark-style.
const THEMES = { ...BUILTIN_THEMES, ...NAMED_THEMES };
 ...
 * `?color=ff6b6b&bg=1a1a2e&border=30363d&text=c9d1d9&dim=8b949e&barBg=21262d&accent2=3fb950`)
 * without picking a whole new theme. Unknown or malformed overrides are ignored
 * so a bad query param can't break the SVG.
 * @param {'dark'|'light'|string} theme
 * @param {{ accent?: string, accent2?: string, bg?: string, border?: string, text?: string, dim?: string, barBg?: string }} [overrides]
 */
 ...
 * `colors` optionally overrides any of the 7 theme keys (hex, no `#`)
...
/**
 * "badge" style: a small pill sized to sit next to a page title or in a
 * header, not shrunk from the full countdown card (which stops being
 * legible below ~half its native size — four boxes of digits need room).
 * Just a label and a big D-day number, the way a Korean site would
 * actually write a small D-day badge by hand.
 */
export function renderCountdownBadge({ title, days, isPast, theme = 'dark', colors, font: fontKey }) {
  const t = resolveThemeOrAuto(theme, colors);
  const font = resolveFont(fontKey);
  const w = 200;
  const h = 60;
